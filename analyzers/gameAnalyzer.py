...
def calculateFlagDistances():
    """
    Calculate max flag distance
    """
...
def myEuclideanDistanceToFlag():
    """
    Calculate my euclidean distance to the wanted flag
    """
...
def actualActivePlayers():
    """
    Update the number of non-dead enemies and allies
    """
    num_enemies = 0
...
def aroundMeSituation(maxWeight):
    """
    Check what's happening around me (enemies, allies, recharge)
    """
    ...
    # (manhattan distance, coordinates)
    ...
    for i in range(startY, endY):
        for j in range(startX, endX):
            ...
            if gameStatus.game.weightedImpostorMap is not None:
                ...
                if gameStatus.game.weightedImpostorMap[i][j] == maxWeight / 2:
                    manhattan = distance.cityblock([myx, myy], [j, i])
                    if nearestEnemy[0] > int(manhattan):
...
def nearestSafeCell():
    """
    Find a position in the map where i can go if I'm in a very dangerous situation
    """
    if gameStatus.game.weightedMap[gameStatus.game.me.y][gameStatus.game.me.x] == 1:
        ...
        if gameStatus.game.weightedMap[gameStatus.game.me.y - 1][gameStatus.game.me.x] == 1:
            ...
            #no safe zone in 2 moves
            gameStatus.game.d_SafeZone[0] = 3


def nextActionsPrediction(model):
    """
    Calculate the prediction looking at other player's actions and update the actual players behaviour
    """
...
class gameAnalyzer(Thread):
    """
    Game analyzer thread. This thread will continously update information about the situation around me and try to infer/predict
    ...
    def run(self):
        ...
        while gameStatus.game.state == "ACTIVE":
            ...
            myEuclideanDistanceToFlag()

            actualActivePlayers()

            aroundMeSituation(self.maxWeight)

            nearestSafeCell()

            nextActionsPrediction(model)
